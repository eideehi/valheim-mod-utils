...
namespace ModUtils
{
    public static class Objects
    {
        ...
        public static List<(Collider collider, T obj, float distance)> GetInsideSphere<T>(
            Vector3 origin, float radius, Func<Collider, T> convertor, Collider[] buffer,
            int layerMask = -1)
        {
            ...
            for (var i = 0; i < size; i++)
            {
                ...
                if (obj != null)
                    result.Add((collider, obj,
                        Vector3.Distance(origin, collider.transform.position)));
            }
        ...
        public static List<(Collider collider, T obj, float distance)> GetInsideSphere<T>(
            Vector3 origin, float radius, Func<Collider, T> convertor, int bufferSize = 128,
            int layerMask = -1)
        {
